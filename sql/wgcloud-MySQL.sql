...
CREATE TABLE `ALARM_INFO` (
                              ...
                              `ALARM_TYPE` char(50) DEFAULT NULL COMMENT '报警类型，如SQL注入、文件上传、DDOS攻击等',
                              `IP_ADDRESS` char(30) DEFAULT NULL COMMENT 'IP地址',
                              `IP_LOCATION` char(100) DEFAULT NULL COMMENT 'IP地理位置信息',
                              `REQUEST_CONTENT` text COMMENT '请求内容，特殊协议的DDOS攻击可不显示',
                              `CREATE_TIME` timestamp NULL DEFAULT NULL COMMENT '创建时间',
                              `IS_BLACKLIST` tinyint(1) NOT NULL DEFAULT 0 COMMENT '黑名单状态（1=是黑名单，0=非黑名单）',
                              PRIMARY KEY (`ID`),
                              INDEX `IDX_IP_BLACKLIST` (`IP_ADDRESS`, `IS_BLACKLIST`)
